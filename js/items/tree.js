class Tree {
    ...
    #generateLevel(point, size) {
        ...
        for (let a = 0; a < Math.PI * 2; a += Math.PI / 16) {
            const kindOfRandom = Math.cos(((a + this.center.x) * size) % 17)**2; // this.center.x: to make trees different from each other | size: to change between levels | a to bring the angles
    ...
    draw(ctx, viewPoint) { // viewPoint to make it look 3D
